...
export interface MarketEvent {
   ...
   * false — событие из нескольких рынков (список строк).
   */
  isBinary: boolean;
  /**
   * Взаимоисключающие ли рынки события (= `negRisk`).
   *
   * true  — рынки отвечают на один вопрос, сработает ровно один: сумма цен
   *         первых исходов ≈ 1, их можно ранжировать по вероятности.
   * false — набор независимых ставок внутри одной темы (пропы матча, серия
   *         «Bitcoin в июле»): сумма цен бессмысленна, рейтинга исходов нет.
   */
  exclusive: boolean;
}
...
/**
 * Ответ пакетного эндпоинта спарклайнов: tokenId -> короткий ряд цен 0..1
 * по возрастанию времени. Токены без пригодной истории в ответ не попадают.
 */
